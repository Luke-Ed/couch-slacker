...
package com.github.luke_ed.couchdb.slacker.viewed;
...
@ContextConfiguration(
    classes = {
      ViewedTestConfiguration.class,
      ViewedCatRepository.class,
      ViewedDogRepository.class,
      ViewedOverviewRepository.class,
      CouchDbInitializer.class
    },
    initializers = ConfigDataApplicationContextInitializer.class)
@ActiveProfiles("viewed-test")
@EntityScan({"com.github.luke_ed.couchdb.slacker.viewed"})
@EnableCouchDbRepositories
class ViewedDocumentsIntegrationTest {

  @Autowired ViewedDogRepository dogRepository;

  @Autowired ViewedCatRepository catRepository;

  @Autowired ViewedOverviewRepository overviewRepository;
  ...
  void testSaveAndViewedFindAll() {
    ...
    assertEquals(
        40,
        overviewRepository.count(),
        "There should be 40 animals in the database (20 dogs + 20 cats)");

    assertEquals(
        20,
        dogRepository.count(),
        "There should be exactly 20 dogs in database. Maybe there is a mismatch in data");
    assertEquals(
        20,
        catRepository.count(),
        "There should be exactly 20 cats in database. Maybe there is a mismatch in data");
  }
