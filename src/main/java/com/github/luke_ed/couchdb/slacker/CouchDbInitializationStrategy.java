...
/**
 * Enum of all com.github.luke_ed.couchdb.slacker.initialization strategies. Strategy determines how
 * the connected CouchDB is initialized. The functionality is provided thru {@link
 * CouchDbInitializer}
 *
 * @author Majlanky
 */
...
public enum CouchDbInitializationStrategy {
  ...
  /** This strategy does no com.github.luke_ed.couchdb.slacker.initialization. */
  NONE(CouchDbInitializationStrategy::noInitialization),
  /**
   * This strategy initializes the connected CouchDB node as single node. If the node is already
   * initialized, the com.github.luke_ed.couchdb.slacker.initialization is skipped.
   */
  SINGLE(CouchDbInitializationStrategy::initializeSingle),
  /**
   * This strategy initializes the configured cluster if the connected node is coordinator ({@link
   * CouchDbProperties.Cluster#isCoordinator()}). If the node is not a coordinator, no
   * com.github.luke_ed.couchdb.slacker.initialization is done, even nodes are configured. If the
   * node is already initialized, the cluster is considered as finished and the
   * com.github.luke_ed.couchdb.slacker.initialization is skipped.
   */
   ...
   */
  CouchDbInitializationStrategy(
      @NotNull ThrowingBiConsumer<CouchDbClient, CouchDbProperties, Exception> initializer) {
   ...
   * @throws Exception if the com.github.luke_ed.couchdb.slacker.initialization fails.
   */
  ...
  private static void noInitialization(CouchDbClient client, CouchDbProperties properties) {
    log.info("No com.github.luke_ed.couchdb.slacker.initialization is done");
  }

  /**
   * @param client must not be {@literal null}
   * @param properties must not be {@literal null}
   * @throws IOException if the com.github.luke_ed.couchdb.slacker.initialization fails.
   */
  private static void initializeSingle(CouchDbClient client, CouchDbProperties properties)
      throws IOException {
    if (!client.databaseExists(USERS_DATABASE_NAME)
        && !client.databaseExists(REPLICATOR_DATABASE_NAME)) {
      ...
      log.info(
          "CouchDB is already initialized, com.github.luke_ed.couchdb.slacker.initialization steps skipped");
    }
  }
   ...
   */
  private static void initializeCluster(CouchDbClient client, CouchDbProperties properties)
    ...
    if (!client.databaseExists(USERS_DATABASE_NAME)
      ...
      if (properties.getCluster().isCoordinator()) {
        ...
        for (String nodeUrl : nodesToAdd) {
          ...
          client.joinNodeToCluster(
              uri.getHost(), uri.getPort(), properties.getUsername(), properties.getPassword());
        }
