...
public enum SchemaOperation {
  ...
  NONE(null, null),
  /**
   * The check that all databases, design documents and views configured in all {@link Document} in
   * scanned packages exists.
   */
  VALIDATE(null, SchemaOperation::validate),
  /**
   * Creates all non-existing databases, design documents and views configured in all {@link
   * Document} in scanned packages. Validation is executed after the creation.
   */
  CREATE(VALIDATE, SchemaOperation::create),
  /**
   * Deletes all existing databases, design documents and views configured in all {@link Document}
   * in scanned packages. Create and validate operations are executed after the dropping. Can be
   * dangerous if databases contains data!
   */
  DROP(CREATE, SchemaOperation::drop);

  @Getter private final SchemaOperation following;
  private final ThrowingBiConsumer<EntityMetadata, CouchDbClient, Exception> action;

  SchemaOperation(
      SchemaOperation following,
      ThrowingBiConsumer<EntityMetadata, CouchDbClient, Exception> action) {
  ...
  private static void create(EntityMetadata metadata, CouchDbClient client) throws IOException {
    ...
    log.info(
        "Checking design {} for {} database", CouchDbClient.ALL_DESIGN, metadata.getDatabaseName());
    DesignDocument allDesign =
        client
            .readDesignSafely(CouchDbClient.ALL_DESIGN, metadata.getDatabaseName())
            .orElseGet(
                () -> {
                  log.info(
                      "Design {} not found in {} database, creating new",
                      CouchDbClient.ALL_DESIGN,
    ...
    if (dataView == null) {
      log.info(
          "View {} not found in design {}, creating new",
          CouchDbClient.ALL_DATA_VIEW,
          CouchDbClient.ALL_DESIGN);
      dataView =
          new View(
              CouchDbClient.ALL_DATA_VIEW, CouchDbClient.ALL_DATA_MAP, CouchDbClient.COUNT_REDUCE);
      allDesign.addView(dataView);
      client.saveDesign(allDesign, metadata.getDatabaseName());
    } else {
      log.info(
          "View {} exists, checking that com.github.luke_ed.couchdb.slacker.mapping and reduce functions matches",
          CouchDbClient.ALL_DATA_VIEW);
      if (!CouchDbClient.ALL_DATA_MAP.equals(dataView.getMapFunction())
          || !CouchDbClient.COUNT_REDUCE.equals(dataView.getReduceFunction())) {
    ...
    if (metadata.isViewed()) {
      log.info(
          "Entities in database {} should be accessed by views and types, going to create design and views if necessary",
          metadata.getDatabaseName());
      DesignDocument design =
          client
              .readDesignSafely(metadata.getDesign(), metadata.getDatabaseName())
              .orElseGet(
                  () -> {
      ...
      if (view == null) {
        ...
        view =
            new View(
                metadata.getView(),
                String.format(CouchDbClient.VIEW_MAP, metadata.getTypeField(), metadata.getType()),
                CouchDbClient.COUNT_REDUCE);
        design.addView(view);
        ...
        log.info(
            "View {} exists, checking that com.github.luke_ed.couchdb.slacker.mapping and reduce functions matches",
            metadata.getView());
        String wantedMapping =
            String.format(CouchDbClient.VIEW_MAP, metadata.getTypeField(), metadata.getType());
        if (!wantedMapping.equals(view.getMapFunction())
            || !CouchDbClient.COUNT_REDUCE.equals(view.getReduceFunction())) {
          log.info("View functions do not match expectation, view will be altered");
  ...
  private static void validate(EntityMetadata metadata, CouchDbClient client)
      throws IOException, SchemaProcessingException {
    log.info("Validating that database {} exists", metadata.getDatabaseName());
    if (!client.databaseExists(metadata.getDatabaseName())) {
      throw new SchemaProcessingException(
          String.format("Database %s does not exists", metadata.getDatabaseName()));
    }
    log.info("Validating that all expected basic design document and views exists");
    Optional<DesignDocument> designAll =
        client.readDesignSafely(CouchDbClient.ALL_DESIGN, metadata.getDatabaseName());
    if (designAll.isEmpty()) {
      throw new SchemaProcessingException(
          String.format(
              "Design '%s' does not exist in %s database",
              CouchDbClient.ALL_DESIGN, metadata.getDatabaseName()));
    }
    View dataView = designAll.get().getViews().get(CouchDbClient.ALL_DATA_VIEW);
    if (dataView == null) {
      throw new SchemaProcessingException(
          String.format(
              "View %s does not exist in design %s of %s database",
    ...
    if (!CouchDbClient.ALL_DATA_MAP.equals(dataView.getMapFunction())
      ...
      throw new SchemaProcessingException(
          String.format(
              "View %s in design %s of %s database contains improper functions",
              CouchDbClient.ALL_DATA_VIEW, CouchDbClient.ALL_DESIGN, metadata.getDatabaseName()));
    }
    if (metadata.isViewed()) {
      log.info(
          "Entities in database {} should be accessed by views and types, validating design and views",
          metadata.getDatabaseName());
      Optional<DesignDocument> design =
          client.readDesignSafely(metadata.getDesign(), metadata.getDatabaseName());
      if (design.isEmpty()) {
        throw new SchemaProcessingException(
            String.format(
                "Design '%s' does not exist in %s database",
