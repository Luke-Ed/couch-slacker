...
import com.github.luke_ed.couchdb.slacker.exception.ClusterException;
...
import com.github.luke_ed.couchdb.slacker.structure.DocumentFindResponse;
...
import com.github.luke_ed.couchdb.slacker.structure.IndexCreateRequest;
...
import com.github.luke_ed.couchdb.slacker.utils.DeleteViewedDocumentSerializer;
import com.github.luke_ed.couchdb.slacker.utils.FoundDocumentDeserializer;
...
public class CouchDbClient {
  ...
  static final String SORTED_TYPED_VIEW_MAP =
      "function(doc){if(doc.%1$s == \"%2$s\"){emit([%3$s]);}}";
   ...
   * @param baseURI where CouchDB is accessible without database specification. Must not be
   *     {@literal null}
   * @param idGenerators {@link Iterable} of available {@link IdGenerator}. If empty, default
   *     generator {@link IdGeneratorUUID} is used. Must not be {@literal null}
  ...
  /**
   * Returns new instance of {@link CouchDbClientBuilder} which is able to build {@link
   * CouchDbClient}
   *
   ...
   * @return {@link org.springframework.data.repository.core.EntityInformation} implementation for
   *     CouchDB entities.
   */
  public <EntityT, IdT> @NotNull CouchDbEntityInformation<EntityT, IdT> getEntityInformation(
      @NotNull Class<EntityT> clazz) {
  ...
  /**
   * Method to obtain new generated id with a relevant {@link IdGenerator}. There is a default
   * generator which is used by default for all document if a document is not annotated with {@link
   * CustomIdGeneration}
   *
   ...
   * @see #CouchDbClient(HttpClient, HttpHost, HttpContext, URI, Iterable, int, int, boolean, int,
   *     QueryStrategy, ObjectMapper, CouchDbContext)
   */
  ...
  /**
   * Method providing wrapping about {@link URIBuilder} which is throwing {@link
   * URISyntaxException}, subtype of {@link Exception}. Because URIes are checked in {@link
   * CouchDbClient} context, there is not need to handle it every time. If URI is not checked {@link
   * IllegalArgumentException} is thrown as warning.
   *
  ...
  /**
   * Saving given entity. If there is no ID for given instance, a relevant {@link IdGenerator} is
  ...
  public <EntityT> @NotNull EntityT save(@NotNull EntityT entity) throws IOException {
    ...
    log.debug(
        "Saving document {} with id {} and revision {} to database {}",
        entity,
        id,
        LazyLog.of(() -> entityMetadata.getRevisionReader().read(entity)),
    ...
    DocumentPutResponse response =
        put(
            getURI(baseURI, entityMetadata.getDatabaseName(), id),
    ...
    log.debug(
        "Saved document {} with id {} and revision {}",
        entity,
        response.getId(),
        response.getRev());
    return entity;
  ...
  /**
   * Saving all given entities in one POST request. If there is no ID for given instance, a relevant
   * {@link IdGenerator} is used to generate new ID. Spring data documentation says, the same list
   * as passed must be returned. Because some updates can fail, it is very unfortunate. The only way
   * to solve this is to check revision and id of returned document to find out, what was
   * saved/updated and what not. The failed documents are logged on warn level.
   *
   ...
   * @return {@link Iterable} of all passed entities with updated revisions and ids. Can not be
   *     {@literal null}
   * @throws IOException if http request is not successful or json processing fail
   */
  @SuppressWarnings({"unchecked"})
  public <EntityT> @NotNull Iterable<EntityT> saveAll(
      @NotNull Iterable<EntityT> entities, @NotNull Class<?> clazz) throws IOException {
    EntityMetadata entityMetadata = getEntityMetadata(clazz);
    log.debug(
        "Bulk save of {} documents to database {}",
        LazyLog.of(() -> StreamSupport.stream(entities.spliterator(), false).count()),
        entityMetadata.getDatabaseName());
    ...
    List<DocumentPutResponse> responses =
        post(
            getURI(baseURI, entityMetadata.getDatabaseName(), "_bulk_docs"),
            localMapper.writeValueAsString(new BulkRequest<>(entities)),
            r ->
                mapper.readValue(
                    r.getEntity().getContent(),
                    mapper
                        .getTypeFactory()
                        .constructCollectionType(List.class, DocumentPutResponse.class)));
    Map<String, DocumentPutResponse> indexed =
        responses.stream().collect(Collectors.toMap(DocumentPutResponse::getId, r -> r));
    for (EntityT e : entities) {
      ...
      if ("true".equals(response.getOk())) {
        ...
        log.warn(
            "Document {} with id: {} and rev: {} saving failed with error {} and reason {}",
            e,
            response.getId(),
            response.getRev(),
            response.getError(),
  ...
  /**
   * Viewed entities needs special treatment during the serialization hence special serializer must
   * be used.
   *
   * @param metadata of the entity. Must not be {@literal null}
   * @param clazz of the entity. Must not be {@literal null}
   * @return the mapper or a mapper which is copy of the global one but with customized
   *     serialization adding the type field.
   */
  private @NotNull ObjectMapper resolveMapper(
      @NotNull EntityMetadata metadata, @NotNull Class<?> clazz) {
    if (metadata.isViewed()) {
      ObjectMapper localMapper = mapper.copy();
      SimpleModule module = new SimpleModule();
      module.addSerializer(
          new ViewedDocumentSerializer<>(clazz, metadata.getTypeField(), metadata.getType()));
   ...
   * @see DocumentBase
   */
  public <EntityT> @NotNull EntityT read(@NotNull String id, @NotNull Class<EntityT> clazz)
      throws IOException {
    String databaseName = getDatabaseName(clazz);
    log.debug("Read of document with ID {} from database {}", id, databaseName);
    return get(
        getURI(baseURI, databaseName, id),
        r -> mapper.readValue(r.getEntity().getContent(), clazz));
  }

  /**
   * Method to get design document of the given name from database which name is obtained from the
   * given class.
   *
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public @NotNull DesignDocument readDesign(@NotNull String id, @NotNull Class<?> clazz)
      throws IOException {
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public @NotNull DesignDocument readDesign(@NotNull String id, @NotNull String databaseName)
      throws IOException {
    log.debug("Read of design with ID {} from database {}", id, databaseName);
    return get(
        getURI(baseURI, databaseName, DESIGN, id),
        r -> mapper.readValue(r.getEntity().getContent(), DesignDocument.class));
  }

  /**
   * Method to get design document of the given name from database which name is obtained from the
   * given class. Differently from {@link #readDesign(String, Class)}, method do not throw
   * IOException in case of {@link HttpStatus#SC_NOT_FOUND} status but returns empty Optional
   * instead.
   *
   * @param id of wanted design document
   * @param clazz from which is obtained database name where design document should be stored
   * @return instance of the wanted {@link DesignDocument} wrapped in {@link Optional} or empty
   *     {@link Optional} is design not found.
   * @throws IOException if http request is not successful or json processing fail
   */
  public @NotNull Optional<DesignDocument> readDesignSafely(
      @NotNull String id, @NotNull Class<?> clazz) throws IOException {
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public Optional<DesignDocument> readDesignSafely(@NotNull String id, @NotNull String databaseName)
      throws IOException {
    try {
  ...
  /**
   * Method for reading all documents of given ids. Read is done in a bulk request. If a id is not
   * found, not entity is returned for the id. Count of ids might not match with count of returned
   * entities.
   *
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull List<EntityT> readAll(
      @NotNull Iterable<String> ids, @NotNull Class<EntityT> clazz) throws IOException {
    ObjectMapper localMapper = mapper.copy();
    SimpleModule module = new SimpleModule();
    module.addDeserializer(List.class, new BulkGetDeserializer<>(clazz, mapper));
    localMapper.registerModule(module);
    log.debug(
        "Bulk read of {} document from database {} with the following IDs: {}",
        LazyLog.of(() -> StreamSupport.stream(ids.spliterator(), false).count()),
        getDatabaseName(clazz),
        LazyLog.of(() -> String.join(", ", ids)));
    BulkGetResponse<EntityT> response =
        post(
            getURI(baseURI, getDatabaseName(clazz), "_bulk_get"),
            localMapper.writeValueAsString(new BulkGetRequest(ids)),
            r ->
                localMapper.readValue(
                    r.getEntity().getContent(),
                    localMapper
                        .getTypeFactory()
                        .constructParametricType(BulkGetResponse.class, clazz)));
    log.info(
        "Bulk read of {} ids result contains {} documents",
        LazyLog.of(() -> StreamSupport.stream(ids.spliterator(), false).count()),
  ...
  /**
   * Method using view to get all document ids. If entity {@link EntityMetadata#isViewed()} than the
   * configured view for the configured design is used. If entity is not
   * com.github.luke_ed.couchdb.slacker.viewed, the expected data view from the all design is used.
   * Method supports pagination. If design documents are needed, use {@link #readAllDesign(Class)}.
   *
   * @param clazz of wanted entity. Used to get database name {@link #getDatabaseName(Class)}. Must
   *     not be {@literal null}
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public @NotNull List<String> readAll(
      @NotNull Class<?> clazz, Long skip, @Nullable Integer limit, @NotNull Sort sort)
      throws IOException {
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public @NotNull List<String> readFromView(
                          ...
                          .orElseThrow(
                              () ->
                                  new IllegalStateException(
                                      "Sorted sort does not contain any order"))
                          .getDirection()
                      == Sort.Direction.DESC
                  ? "true"
                  : "false"));
    ...
    return get(
        getURI(baseURI, Arrays.asList(database, DESIGN, design, VIEW, view), parameters),
  ...
  private Pair<String, String> getSortedViewId(Sort sort, EntityMetadata em) throws IOException {
    String sortViewId =
        "sorted-by-"
            + (em.isViewed() ? em.getType() + "-" : "")
            + sort.stream()
                .map(o -> o.getProperty().replace(".", "-"))
    ...
    if (!knownSortedViews.contains(sortViewId)) {
      if (!design.getViews().containsKey(sortViewId)) {
        View view;
        String sortKey =
            sort.stream().map(o -> "doc." + o.getProperty()).collect(Collectors.joining(","));
        if (em.isViewed()) {
          view =
              new View(
                  sortViewId,
                  String.format(SORTED_TYPED_VIEW_MAP, em.getTypeField(), em.getType(), sortKey),
                  COUNT_REDUCE);
        } else {
  ...
  /**
   * Method tests if the two given directions are the same. The test is necessary because CouchDB is
   * not able to mix sort directions in Mango query.
   *
   ...
   */
  public static @NotNull Sort.Direction assertSameDirection(
      @Nullable Sort.Direction expected, @NotNull Sort.Direction current) {
  ...
  /**
   * Method using view to get document count. If entity {@link EntityMetadata#isViewed()} than the
   * configured view for the configured design is used. If entity is not
   * com.github.luke_ed.couchdb.slacker.viewed, the expected data view from the all design is used.
   *
   * @param clazz of wanted entity. Used to get database name {@link #getDatabaseName(Class)}. Must
  ...
  public long countAll(@NotNull Class<?> clazz) throws IOException {
    ...
    return get(
        getURI(baseURI, em.getDatabaseName(), DESIGN, design, VIEW, view),
        r -> {
          JsonNode rows =
              mapper.readValue(r.getEntity().getContent(), ObjectNode.class).get("rows");
          if (rows.has(0)) {
   ...
   * @return instance of the given {@code resultClass} with mapped result of the requested reduce
   *     function.
   * @throws IOException if http request is not successful or json processing fail
   */
  public <ResultT> ResultT reduce(
      @NotNull String database,
      @NotNull String design,
      @NotNull String view,
      ...
      throws IOException {
    return get(
        getURI(baseURI, database, DESIGN, design, VIEW, view),
        r -> {
          JsonNode rows =
              mapper.readValue(r.getEntity().getContent(), ObjectNode.class).get("rows");
  ...
  /**
   * Method to get result of _design_docs which contains only design documents (If you need
   * non-design documents use {@link #readAll(Class)} if you want both) to the database specified by
   * {@link Document} from the passed entity class. Result is returned as Stream of documents ids,
   * no full data.
   *
   * @param clazz of wanted entity. Used to get database name {@link #getDatabaseName(Class)}. Must
   *     not be {@literal null}
  ...
  public @NotNull List<String> readAllDesign(@NotNull Class<?> clazz) throws IOException {
    ...
    return get(
        getURI(baseURI, databaseName, "_design_docs"),
        r -> mapper.readValue(r.getEntity().getContent(), AllDocumentResponse.class).getRows());
  }
  ...
  public <EntityT> @NotNull EntityT delete(@NotNull EntityT entity) throws IOException {
    ...
    log.debug(
        "Delete of document with id {} and revision {} from database {}",
        id,
        revision,
        entityMetadata.getDatabaseName());
    return delete(
        getURI(
            baseURI,
            Arrays.asList(entityMetadata.getDatabaseName(), id),
            Collections.singletonList(new BasicNameValuePair("rev", revision))),
        r -> entity);
  }

  /**
   * Method to remove all documents from database. Method is implemented as read all ids and than
   * delete by id. Both mentioned functions are bulk operations. As a consequence of the mentioned
   * approach, DB deletes only document existing in the time of call, not documents created after
   * the request . Name of the database in which delete is executed is read from given class.
   *
   * @param clazz with {@link Document} annotation
   * @param <EntityT> Type of entity in the database
   * @return {@link List} of deleted documents
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull List<EntityT> deleteAll(@NotNull Class<EntityT> clazz)
      throws IOException {
  ...
  public <EntityT> @NotNull List<EntityT> deleteAll(
      @NotNull Iterable<EntityT> entities, @NotNull Class<?> clazz) throws IOException {
    EntityMetadata entityMetadata = getEntityMetadata(clazz);
    log.debug(
        "Bulk delete of {} documents from database {}",
        LazyLog.of(() -> StreamSupport.stream(entities.spliterator(), false).count()),
        entityMetadata.getDatabaseName());
    ObjectMapper localMapper = mapper.copy();
    SimpleModule module = new SimpleModule();
    module.addSerializer(
        entityMetadata.isViewed()
            ? new DeleteViewedDocumentSerializer<>(
                clazz, entityMetadata.getTypeField(), entityMetadata.getType())
    ...
    for (EntityT e : entities) {
      ...
      if ("true".equals(response.getOk())) {
        ...
        log.warn(
            "Document {} with id: {} and rev: {} deleting failed with error {} and reason {}",
            e,
            response.getId(),
            response.getRev(),
            response.getError(),
  ...
  /**
   * Deletes document from database based on {@link Document} annotation of given {@code clazz} with
   * the given id. The method deletes latest document, without any consistency check.
   *
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull EntityT deleteById(@NotNull String id, @NotNull Class<EntityT> clazz)
      throws IOException {
    EntityMetadata entityMetadata = getEntityMetadata(clazz);
    ...
    return delete(
        getURI(
            ...
            Collections.singletonList(
                new BasicNameValuePair("rev", entityMetadata.getRevisionReader().read(entity)))),
        r -> entity);
  }

  /**
   * Executes the given Mango query and maps a result into the given entity. To better performance
   * index is needed. If mango query is created from generic query method {@link Index} annotation
   * can be used to specify the index(es).
   *
   ...
   * @param <EntityT> type of entity
   * @return pair of bookmark of result and {@link List} of instances of the given class with result
   *     of the given class
   * @throws IOException if http request is not successful or json processing fail
   * @see DocumentBase
   */
  public <EntityT> @NotNull Pair<List<EntityT>, String> find(
      @NotNull String json, @NotNull Class<EntityT> clazz) throws IOException {
    ObjectMapper localMapper = mapper.copy();
    ...
    DocumentFindResponse<EntityT> response =
        post(
            getURI(baseURI, getDatabaseName(clazz), "_find"),
            json,
            r ->
                localMapper.readValue(
                    r.getEntity().getContent(),
                    localMapper
                        .getTypeFactory()
                        .constructParametricType(DocumentFindResponse.class, clazz)));
   ...
   *
   * @param request that will be executed depending on query strategy configuration. Must not be
   *     {@literal null}
   ...
   * @return {@link FindResult} with entities matching the provided request and bookmarks if the
   *     configured strategy returns it.
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull FindResult<EntityT> find(
      @NotNull FindRequest request, @NotNull Class<EntityT> clazz) throws IOException {
    return find(request, clazz, null);
  }

  /**
   * Executes the given request as mango or view lookup. If configured query strategy provides
   * bookmarks, the {@code bookmarkBy} parameter is used as size of page for that bookmark will be
   * generated.
   *
   * @param request that will be executed depending on query strategy configuration. Must not be
   *     {@literal null}
   * @param clazz that will be used to obtain database name. Must not be {@literal null}
   * @param bookmarkBy number of how many documents should be between two bookmarks. Affects
   *     performance, null turns of chunking to bookmarks
   * @param <EntityT> type of entities that should be in a result of query
   * @return {@link FindResult} with entities matching the provided request and bookmarks if the
   *     configured strategy returns it.
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull FindResult<EntityT> find(
      @NotNull FindRequest request, @NotNull Class<EntityT> clazz, @Nullable Integer bookmarkBy)
      throws IOException {
    QueryStrategy pickedStrategy =
        request.getQueryStrategy() != null ? request.getQueryStrategy() : queryStrategy;
  ...
  /**
   * Method used if {@link QueryStrategy#VIEW} is configured. Method uses {@link #ensureView(Sort,
   * String, Class)} method to create (or obtain the existing one) view where
   * com.github.luke_ed.couchdb.slacker.mapping function is matching the provided request. The view
   * is used as source of a result.
   *
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull FindResult<EntityT> findByView(
      @NotNull FindRequest request, @NotNull Class<EntityT> clazz) throws IOException {
  ...
  /**
   * Method to obtain total amount of documents matching the given request. It depends on the
   * configured query strategy if mango or view is used to process the given request.
   *
   * @param request that will be executed. Must not be {@literal null}
   * @param clazz that will be used to obtain database name. Must not be {@literal null}
  ...
  /**
   * Method to obtain total amount of documents matching the given request. Method uses {@link
   * #ensureView(Sort, String, Class)} method to create (or obtain the existing one) view where
   * com.github.luke_ed.couchdb.slacker.mapping function is matching the provided request. The view
   * is used in reduce mode with _count to get total.
   *
   * @param request that will be executed. Must not be {@literal null}
   * @param clazz that will be used to obtain database name. Must not be {@literal null}
   * @return total amount of entities matching the given request
   * @throws IOException if http request is not successful or json processing fail
   */
  public long countByView(@NotNull FindRequest request, @NotNull Class<?> clazz)
      throws IOException {
    String designId = ensureView(request.getSort(), request.getJavaScriptCondition(mapper), clazz);
    log.debug("Using design {} for counting {}", designId, request);
    return get(
        getURI(baseURI, getDatabaseName(clazz), DESIGN, designId, VIEW, ALL_DATA_VIEW),
        r -> {
          JsonNode rows =
              mapper.readValue(r.getEntity().getContent(), ObjectNode.class).get("rows");
          if (rows.has(0)) {
  ...
  /**
   * Method to ensure that a view matching the given javascript condition and sort exists or will be
   * created. Sort is used to determine the key of view. If there is not sort, view emits null. The
   * given javascript is used in com.github.luke_ed.couchdb.slacker.mapping function of the view.
   *
   * @param sort of the view. Must not be {@literal null}
   * @param javaScriptCondition of com.github.luke_ed.couchdb.slacker.mapping function of the view.
   *     Must not be {@literal null}
   * @param clazz that will be used to obtain database name. Must not be {@literal null}
   * @return name of the view with com.github.luke_ed.couchdb.slacker.mapping function matching the
   *     given javascript condition and key(s) with the given sort.
   * @throws IOException if http request is not successful or json processing fail
   */
  private String ensureView(
      @NotNull Sort sort, @NotNull String javaScriptCondition, @NotNull Class<?> clazz)
      throws IOException {
    String mapFunction;
    if (sort.isSorted()) {
      Sort.Direction direction = null;
    ...
    if (!design.isPresent()) {
      log.debug(
          "Creating new view {} in design {} for querying with map function {}",
          ALL_DATA_VIEW,
          mapFunction.hashCode() + "",
          mapFunction);
      View view = new View(ALL_DATA_VIEW, mapFunction, COUNT_REDUCE);
      DesignDocument newDesign =
          new DesignDocument(mapFunction.hashCode() + "", Collections.singleton(view));
  ...
  /**
   * Method used if {@link QueryStrategy#MANGO} is configured. Method uses translates the given
   * request to mango json and executes it.
   *
   * @param request that will be executed. Must not be {@literal null}
   * @param clazz that will be used to obtain database name. Must not be {@literal null}
   * @param bookmarkBy number of how many documents should be between two bookmarks. Affects
   ...
   * @throws IOException if http request is not successful or json processing fail
   */
  public <EntityT> @NotNull FindResult<EntityT> findByMango(
      @NotNull FindRequest request, @NotNull Class<EntityT> clazz, @Nullable Integer bookmarkBy)
      throws IOException {
    ...
    do {
      // if we are limited by request and we can see that next request cause overflow of the limit,
      // we request as less as needed.
      ...
      // We repeat whole process with next bookmark until we have all documents or as much document
      // as limit from request
    } while ((originalLimit != null
            && result.size() < originalLimit
            && r.getFirst().size() == limit)
  ...
  /**
   * Method to create new index by the given parameters. All order rules must be in the same
   * direction (it is a limitation of CouchDB)
   *
   * @param name of the created index. Must not be {@literal null}
   * @param entityClass as definition of database in which index should be created. Must not be
   *     {@literal null}
   * @param fields list of {@link org.springframework.data.domain.Sort.Order} which from index
   *     should be done
   * @throws IOException if http request is not successful or json processing fail
   */
  public void createIndex(@NotNull String name, @NotNull Class<?> entityClass, Sort.Order... fields)
      throws IOException {
    createIndex(
        name, entityClass, fields == null ? Collections.emptyList() : Arrays.asList(fields));
  }
   ...
   *     {@literal null}
   * @param fields list of {@link org.springframework.data.domain.Sort.Order} which from index
   *     should be done. Must not be {@literal null}
   * @throws IOException if http request is not successful or json processing fail
   */
  public void createIndex(
      @NotNull String name, @NotNull Class<?> entityClass, @NotNull Iterable<Sort.Order> fields)
      throws IOException {
   ...
   */
  public void createIndex(
      @NotNull String name, @NotNull String dbName, @NotNull Iterable<Sort.Order> fields)
      throws IOException {
    Sort.Direction direction = null;
    ...
    log.debug(
        "Creating index with name {} in database {} and ordering {}",
        name,
        dbName,
        LazyLog.of(
            () ->
                StreamSupport.stream(fields.spliterator(), false)
                    ...
                    .collect(Collectors.joining(", "))));
    post(
        getURI(baseURI, dbName, "_index"),
        mapper.writeValueAsString(new IndexCreateRequest(name, fields)),
        r -> null);
  }
   ...
   * @param clazz as definition of database in which index should be created. Must not be {@literal
   *     null}
   * @throws IOException if http request is not successful or json processing fail
   */
  private void createIndex(@NotNull Sort sort, @NotNull Class<?> clazz) throws IOException {
    ...
    String indexId =
        sort.stream().map(Sort.Order::getProperty).collect(Collectors.joining("-"))
            + "-"
            + sort.stream()
                .findFirst()
                .orElseThrow(
                    () -> new IllegalStateException("Sorted sort does not contain any order"))
                .toString()
                .toLowerCase();
  ...
  public void createDatabase(
      @NotNull Class<?> clazz, int shardsCount, int replicasCount, boolean partitioned)
      throws IOException {
   ...
   */
  public void createDatabase(
      @NotNull String name, int shardsCount, int replicasCount, boolean partitioned)
      throws IOException {
    log.debug(
        "Creating {} database with name {}, {} shards, {} replicas",
        LazyLog.of(() -> partitioned ? "portioned" : "non-portioned"),
        name,
        shardsCount,
        replicasCount);
    put(
        getURI(
            ...
            Arrays.asList(
                new BasicNameValuePair("q", "" + shardsCount),
                new BasicNameValuePair("n", replicasCount + ""),
  ...
  /**
   * Method for storing a design document to the database which is obtained from the given class. ID
   * of the given design document must not be {@literal null} nor empty.
   *
   * @param designDocument which should be saved. Must not be {@literal null}
   * @param clazz which is used as source of database name where to store the given document. Must
   *     not be {@literal null}
   * @return Stored {@link DesignDocument} with updated id and revision
   * @throws IOException if http request is not successful
   */
  public @NotNull DesignDocument saveDesign(DesignDocument designDocument, Class<?> clazz)
      throws IOException {
    return saveDesign(designDocument, getDatabaseName(clazz));
  }

  /**
   * Method for storing a design document to the database with the given name. ID of the given
   * design document must not be {@literal null} nor empty.
   *
   * @param designDocument which should be saved. Must not be {@literal null}
   ...
   * @throws IOException if http request is not successful
   */
  public @NotNull DesignDocument saveDesign(
      @NotNull DesignDocument designDocument, @NotNull String databaseName) throws IOException {
    Assert.hasText(designDocument.getId(), "DesignDocument.Id must not be null");
    log.debug(
        "Saving design with id {} and revision {} to database {}",
        designDocument.getId(),
        designDocument.getRevision(),
        databaseName);
    DocumentPutResponse response =
        put(
            getURI(baseURI, databaseName, designDocument.getId()),
            mapper.writeValueAsString(designDocument),
            r -> mapper.readValue(r.getEntity().getContent(), DocumentPutResponse.class));
  ...
  /**
   * @return {@link Optional#of(Object)} an UUID of the connected CouchDB. If no UUID is set, {@link
  ...
  public @NotNull Optional<String> getUUID() throws IOException {
    ...
    try {
      uuid =
          Optional.of(
              get(
                  getURI(baseURI, NODE, LOCAL, "_config", "couchdb", "uuid"),
  ...
  public void setUUID(@NotNull String uuid) throws IOException {
    put(
        getURI(baseURI, NODE, LOCAL, "_config", "couchdb", "uuid"),
        mapper.writeValueAsString(uuid),
        r -> null);
  }

  /**
   * Method joins node of the given host and port to the cluster thru the connected CouchDB. This
   * can be used only if the connected CouchDB is coordinator of the cluster.
   *
   ...
   * @throws IOException if http request is not successful
   */
  public void joinNodeToCluster(
      @NotNull String host, int port, @NotNull String username, @NotNull String password)
      throws IOException {
  ...
  /**
   * Method sends request to finish cluster setting. This can be used only if the connected CouchDB
   * is coordinator of the cluster.
   *
   * @throws IOException if http request is not successful
   */
  ...
  /**
   * Method to get node id (name) of the connected CouchDB. Used as recognition of the connected
   * CouchDB during cluster setup.
   *
  ...
  public @NotNull String getNodeId() throws IOException {
    return get(
        getURI(baseURI, NODE, LOCAL),
        r -> mapper.readValue(r.getEntity().getContent(), JsonNode.class).get("name").asText());
  }
  ...
  /**
   * Method to send PUT request with the given body with possibility to safely process response.
   * HTTP response is closed immediately after {@code responseProcessor} is called. Body is expected
   * as json, content type is hard coded as application/json.
   *
   * @param uri of target. Must not be {@literal null}
   * @param json body of the request. Must not be {@literal null}
   * @param responseProcessor {@link ThrowingFunction} to process response before the stream is
   *     closed. Must not be {@literal null}
   * @param <DataT> type of returned data which are created by {@code responseProcessor}
   * @return data created in {@code responseProcessor} based on response of PUT request with the
   *     given {@code json} on the given {@code uri}
   * @throws IOException if http request is not successful or json processing fail
   */
  private <DataT> DataT put(
      @NotNull URI uri,
      @NotNull String json,
      @NotNull ThrowingFunction<HttpResponse, DataT, IOException> responseProcessor)
      throws IOException {
  ...
  /**
   * Method to send POST request with the given body with possibility to safely process response.
   * HTTP response is closed immediately after {@code responseProcessor} is called. Body is expected
   * as json, content type is hard coded as application/json.
   *
   * @param uri of target. Must not be {@literal null}
   ...
   *     closed. Must not be {@literal null}
   * @param <DataT> type of returned data which are created by {@code responseProcessor}
   * @return data created in {@code responseProcessor} based on response of POST request with the
   *     given {@code json} on the given {@code uri}
   * @throws IOException if http request is not successful or json processing fail
   */
  private <DataT> DataT post(
      @NotNull URI uri,
      @NotNull String json,
      @NotNull ThrowingFunction<HttpResponse, DataT, IOException> responseProcessor)
      throws IOException {
  ...
  /**
   * Method to send GET request with possibility to safely process response. HTTP response is closed
   * immediately after {@code responseProcessor} is called. Content type is hard coded as
   * application/json.
   *
   * @param uri of target. Must not be {@literal null}
   * @param responseProcessor {@link ThrowingFunction} to process response before the stream is
