...
package com.github.luke_ed.couchdb.slacker.integration.initialization;
...
class ClusterInitializationIntegrationTest {
    ...
    private static final Map<@NotNull String, @NotNull String> sharedEnvironment = Map.of(
            "COUCHDB_USER", "admin",
    ...
    @Container
    static GenericContainer<?> couchDbContainer0 = new GenericContainer<>(DockerImageName.parse("couchdb:latest"))
            .withEnv(
                    sharedEnvironment
            )
    ...
    @Container
    static GenericContainer<?> couchDbContainer1 = new GenericContainer<>(DockerImageName.parse("couchdb:latest"))
            .withEnv(
                    sharedEnvironment
            )
    ...
    @Container
    static GenericContainer<?> couchDbContainer2 = new GenericContainer<>(DockerImageName.parse("couchdb:latest"))
    ...
    static void dynamicProperties(DynamicPropertyRegistry registry) {
        registry.add(
                "couchdb.client.url",
                () -> "http://%s:%d".formatted(couchDbContainer0.getHost(), couchDbContainer0.getFirstMappedPort())
        );
